...
const Manager = ({isAuthenticated}) => {
  ...
  const fetchPasswords = async () => {
    ...
    try {
      const response = await axios.get(import.meta.env.VITE_BASE_ADDRESS +'getpasswords', {
  ...
  const handleDelete = async (passwordId) => {
    const token = localStorage.getItem('token');
    try {
      const response = await axios.delete(`${import.meta.env.VITE_BASE_ADDRESS}deletepassword/${passwordId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.data.success) {
        setdataArray(dataArray.filter(password => password._id !== passwordId)); // Update UI after deletion
  ...
  return (
    <>
      ...
      <div className=" mx-auto max-w-4xl h-[calc(100% - 88px)] p-4 flex flex-col gap-4 ">
        <h1 className="text-[#43287d] underline underline-offset-4 drop text-center text-lg font-bold">
        ...
        <div className="flex flex-col p-4 gap-8  ">
          ...
          <div className="m-auto">
            <button className="addpassBtn flex justify-around">
              <img className="w-6  text" src={addicon} alt="" />
              <span
                onClick={savePassword}
                className="addpasstxt text-[#eb0e0edd] px-4 font-bold"
              >
                Save
              </span>
            </button>
